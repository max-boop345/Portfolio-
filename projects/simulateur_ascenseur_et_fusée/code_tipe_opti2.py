...
# -*- coding: utf-8 -*-
"""
Created on Sun Dec  8 12:30:51 2024
...
def make_orient_event(theta_target):
    def est_oriente(t, x):
        theta_actual = np.arctan2(x[1], x[0])
        return theta_actual - theta_target
    est_oriente.terminal = False
    est_oriente.direction = 0
    return est_oriente 

def make_orient_event_2(liste):
    est_oriente = []
    for theta_target in liste:
        def template(t, x, theta_target=theta_target):
            theta_actual = np.arctan2(x[1], x[0])
            return theta_actual - theta_target
        template.terminal = False
        template.direction = 0
        est_oriente.append(template)
    return est_oriente
...
def satelisation(traj3):
    ...
    return etat_min, t_start, n

def integrate(CI, t_event,theta):      #pour satelisé en 2 temps
    traj3 = solve_ivp(rhs2, 
                      ...
                      events=[est_parr_a_TL_dans_le_sens_plus_ux,
                              ...
                              est_oriente[theta0_values.tolist().index(theta)],
...
def integrate2(CI, t_event):     #pour aller vers la lune
    traj3 = solve_ivp(rhs2, 
                      (t_event, TMAX), 
                      CI, 
                      method='RK45',
...
def integrate3(CI, t_start):     #pour intégrer de manière précise
    traj3 = solve_ivp(rhs2, 
                      (t_start, TMAX), 
                      CI, 
                      method='DOP853',
...
def concat_res(traj3):
    ...
    valeur_x = np.hstack([trajectoire_initiale_x[:], valeur_x])
    valeur_t = np.hstack([trajectoire_initiale_t[:], valeur_t])
    return valeur_x, valeur_t

def concat_res2(traj3):
    valeur_x, valeur_t = [], []  
    # Stocker uniquement les résultats de la trajectoire circulaire
    valeur_x.append(traj3.y)
    valeur_t.append(traj3.t)
...
theta0_values = np.linspace(theta0, theta0 + np.pi/8, p)
...
est_oriente = make_orient_event_2(theta0_values)

for i in points:
    angle_départ =-np.pi/4
    theta = i[0]
    r_ini = i[1]
    impulsion = i[2]
    r_peri = i[3]
    ...
    x0,y0=-alpha+r_ini*np.cos(angle_départ),r_ini*np.sin(angle_départ)
    vx0,vy0=-v0*np.sin(angle_départ),v0*np.cos(angle_départ)
    ...
    traj3=integrate(CI, t_start,theta)
    valeur_x.append(traj3.y)
    valeur_t.append(traj3.t)
    ...
    if traj3.t_events[0].size>0:
        ...
        CI, t_eval = parra_plus(traj3)
        

        #nouvelle intégration avec les nouvelles CI et domaine temp
        traj3=integrate(CI, t_eval,theta)
        valeur_x, valeur_t = [], []  
        valeur_x, valeur_t = concat_res(traj3)
    ...
    if traj3.t_events[0].size>1:        #attention, ici les events ont été modifié : parra+ et parra- ont été enlevé
        ...
        valeur_x, valeur_t = [], []  
        valeur_x, valeur_t = concat_res2(traj3)
...
with open("simulation_results.pkl", "wb") as f:
    pickle.dump({"val_x": val_x, "val_t": val_t, "distance_satelisation_atteint": distance_satelisation_atteint}, f)
